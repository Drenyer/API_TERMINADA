  ...
  try {
    ...
    if (fecha) {
      if (params.FilterExpression) {
        params.FilterExpression += ' AND fechaCreacion = :fecha';
      } else {
        params.FilterExpression = 'fechaCreacion = :fecha';
      }
      params.ExpressionAttributeValues[':fecha'] = fecha;
    }

    if (!params.FilterExpression) {
      return res.status(400).json({ error: 'Se requiere al menos un filtro' });
    }

    const result = await dynamoDB.scan(params).promise();
    console.log('Productos devueltos del backend:', result.Items); // Agrega este log
